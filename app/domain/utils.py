...
def save_text(text_information: str, file_name: str, index: Optional[int] = None) -> Tuple[int,str]:
    full_path = f"tmp/{file_name}.txt"
    with open(full_path, "w", encoding="utf-8") as text_file:
        text_file.write(text_information)
        return index, full_path

def parallel_save_texts(text_list: List[str],  max_workers:  Optional[int] = 40) -> List[str]:

    args_list = [
        (
            content, f"text_{index}", index
        )
        for index, content in enumerate(text_list)
    ]
    result_process = []

    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = [
                    executor.submit(save_text, *args) 
                    for args in args_list
                ]
        
        with tqdm(total=len(text_list), desc="Saving texts", unit="text") as pbar:
            for future in as_completed(futures):
                result_process.append(future.result())
                pbar.update(1)

    return [ result for _, result in sorted(result_process, key=lambda x: x[0]) ]    
...
def divide_text_by_worker(message: str, parallel_workers: Optional[int] = 10, overlap: Optional[int] = 10) -> List[str]:
    total_length_message = len(message)
    new_parts_dimension = mt.ceil(total_length_message/parallel_workers)
    new_text_parts = []

    if new_parts_dimension < 2:
        return [ message ]
    
    for part in range(new_parts_dimension):
        if part < 1:
            end_index = min((part+1)*parallel_workers + overlap, total_length_message)
            new_text_parts.append(message[part*parallel_workers:end_index])

        elif part == (new_parts_dimension - 1):
            start_index = max(part*parallel_workers-overlap, 0) 
            new_text_parts.append(message[start_index:])
        else:
            start_index = part*parallel_workers - overlap
            end_index = (part+1)*parallel_workers + overlap
            new_text_parts.append(message[start_index:end_index])
    
    return new_text_parts

def divide_text_by_words(message: str, max_words: Optional[int] = 500, overlap: Optional[int] = 1) -> List[str]:
    words = message.split()
    ...
    total_parts_dim = mt.ceil(total_words/max_words)
    new_text_parts = []

    for part in range(total_parts_dim):
        if part < 1:
            end_index= min((part+1)*max_words + overlap, total_words)
            new_text_parts.append(' '.join(words[part:end_index]))
        elif part == (total_parts_dim - 1):
            start_index = max(part*max_words-overlap, 0)
            new_text_parts.append(' '.join(words[start_index:]))
        else:
            start_index = max(part*max_words - overlap, 0)
            end_index = min((part+1)*max_words + overlap, total_words)
            new_text_parts.append(' '.join(words[start_index:end_index]))
    return new_text_parts

def divide_text(message: str, parallel_workers: Optional[int] = 10, overlap: Optional[int] = 5) -> List[str]:
    total_parts = divide_text_by_words(message)
    if len(total_parts) > parallel_workers:
        return divide_text_by_worker(message, parallel_workers, overlap)
